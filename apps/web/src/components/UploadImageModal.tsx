...
export function UploadImageModal({ open, onClose, onSave, title }: UploadImageModalProps) {
  ...
  const handleUploadSave = useCallback(async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : t('image.uploadFailed', 'Upload failed'));
    } finally {
      setLoading(false);
    }
  }, [file, onSave, onClose, t]);
  ...
  const footer = (
    <>
      ...
      <Button disabled={!file || loading} onClick={handleUploadSave}>
        {loading ? t('image.uploading', 'Uploading…') : t('image.uploadSave', 'Upload & Save')}
      </Button>
    </>
  );
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={title ?? t('image.uploadTitle', 'Upload image')}
      footer={footer}
      className="max-w-md"
    >
